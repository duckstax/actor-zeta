            ...
            static std::atomic<memory_resource*> __res =
                {reinterpret_cast<memory_resource*>(
                    reinterpret_cast<std::uintptr_t*>(
                        &instance_))};
            if (new_res) {
                std::printf("%s :: &instance_ = %p; new_res = %p; __res = %p\n", __func__, &instance_, new_res, __res.load());
                auto ret = std::atomic_exchange_explicit(
                    &__res, new_res, std::memory_order_acq_rel); // @TODO check memory order
                std::printf("%s :: &instance_ = %p; new_res = %p; ret = %p; __res = %p\n", __func__, &instance_, new_res, ret, __res.load());
                return ret;
            }
