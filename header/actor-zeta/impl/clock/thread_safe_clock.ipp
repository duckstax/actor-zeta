...
namespace actor_zeta { namespace clock {
    ...
    void thread_safe_clock_t::schedule_periodically(time_point first_run, handler f,
                                                   duration_type period) {
        auto ptr = schedule_entry_ptr{new schedule_entry{first_run, std::move(f), period}};
    ...
    void thread_safe_clock_t::run() {
        ...
        while (running_) {
            ...
            //tbl_.erase(std::remove_if(tbl_.begin(), tbl_.end(), is_disposed),tbl_.end());
        }
    }
