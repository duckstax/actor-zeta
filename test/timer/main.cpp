...
TEST_CASE("timer") {
    ...
    auto supervisor = actor_zeta::spawn_supervisor<supervisor_lite>(mr_ptr, mr_ptr);

    auto& clock = supervisor->clock();
    auto time = clock.now() + std::chrono::seconds(10);
    clock.schedule_message(time, supervisor->address(), actor_zeta::make_message(mr_ptr, actor_zeta::address_t::empty_address(), alarm_id));
