...
public:
    dispatcher_t(manager_dispatcher_t* ptr, std::string name,actor_zeta::address_t manager_database )
        ...
        count_actor++;
        add_handler(command_t::create_database, &dispatcher_t::create_database);
        add_handler(command_t::create_collection, &dispatcher_t::create_collection);
        add_handler(command_t::insert, &dispatcher_t::insert);
        add_handler(command_t::add_link,&dispatcher_t::add_link);
        add_handler(command_t::add_address,&dispatcher_t::add_address);
    }
    void insert(std::string& name, int key, int value) {
        ...
        actor_zeta::send(collection, address(), command_t::insert, key, value);
    }

    void create_database(std::string& name) {
        std::cerr << "dispatcher_t::create_database : " << name << std::endl;
        actor_zeta::send(manager_database_, address(), command_t::create, std::move(name));
    }
    ...
    void create_collection(std::string& name_database, std::string& name_collection) {
        ...
        actor_zeta::send(database, address(), command_t::create, std::move(name_collection));
    }
...
public:
    collection_t(database_t* ptr, std::string& name)
        : actor_zeta::basic_async_actor(ptr, std::move(name)) {
        count_actor++;
        add_handler(command_t::insert, [this](int key, int value) {
            std::cerr << "collection_t::insert : key : " << key << " value : " << value << std::endl;
            storage_.emplace(key, value);
        });
        add_handler(command_t::add_link,&collection_t::add_link);
    }
...
public:
    ...
    void create(std::string& name) {
        ...
        actor_zeta::send(current_message()->sender(),address(),command_t::add_address,tmp);
...
public:
    manager_database_t(memory_resource* mr)
        ...
        , e_(new actor_zeta::scheduler_t<actor_zeta::work_sharing>(
                 ...
                 100),
             thread_pool_deleter){
        add_handler(command_t::create, &manager_database_t::create);
        add_handler(command_t::add_link,&manager_database_t::add_link);
        e_->start();
        count_supervisor++;
    }
    ...
    void create(std::string& name) {
        ...
        actor_zeta::send(current_message()->sender(),address(),command_t::add_address,db);
...
database_t::database_t(manager_database_t* ptr, std::string name)
    ...
    , e_(ptr->scheduler()) {
    add_handler(command_t::create, &database_t::create);
    add_handler(command_t::add_link,&database_t::add_link);
    count_supervisor++;
}
...
public:
    manager_dispatcher_t(memory_resource* mr)
        ...
        , e_(new actor_zeta::scheduler_t<actor_zeta::work_sharing>(
                 ...
                 100),
             thread_pool_deleter){
        add_handler(command_t::create, &manager_dispatcher_t::create);
        add_handler(command_t::add_link,&manager_dispatcher_t::add_link);
        add_handler(command_t::create_database, &manager_dispatcher_t::create_database);
        add_handler(command_t::create_collection, &manager_dispatcher_t::create_collection);
        add_handler(command_t::insert, &manager_dispatcher_t::insert);
        e_->start();
        count_supervisor++;
    }
    void insert( std::string& database,std::string& name, int key, int value) {
        actor_zeta::send(dispathers_[0], address(), command_t::insert,std::move(database), std::move(name), key, value);
    }

    void create_database(std::string& name) {
        std::cerr << "dispatcher_t::create_database : " << name << std::endl;
        actor_zeta::send(dispathers_[0], address(), command_t::create_database, std::move(name));
    }
    ...
    void create_collection(std::string& name_database, std::string& name_collection) {
        std::cerr << "dispatcher_t::create_collection : "
                  ...
                  << " collection : " << name_collection
                  << std::endl;
        actor_zeta::send(dispathers_[0], address(), command_t::create_collection, std::move(name_database), std::move(name_collection));
    }
    ...
    void create(std::string& name) {
        ...
        actor_zeta::send(address_book_.at("mdb"), dispather,command_t::add_link);
...
auto main() -> int {
    ...
    actor_zeta::send(manager_dispatcher, manager_database->address(), command_t::add_link);
    std::this_thread::sleep_for(sleep_milliseconds_time);
    actor_zeta::send(manager_database, manager_dispatcher->address(), command_t::add_link);
    std::this_thread::sleep_for(sleep_milliseconds_time);
    actor_zeta::send(manager_dispatcher, actor_zeta::address_t::empty_address(), command_t::create, std::string("dispatcher"));
    std::this_thread::sleep_for(sleep_milliseconds_time);
    actor_zeta::send(manager_dispatcher, actor_zeta::address_t::empty_address(), command_t::create_database, std::string("database_test"));
    std::this_thread::sleep_for(sleep_milliseconds_time);
    actor_zeta::send(manager_dispatcher, actor_zeta::address_t::empty_address(), command_t::create_collection, std::string("database_test"), std::string("collection_test"));
    std::this_thread::sleep_for(sleep_milliseconds_time);
    actor_zeta::send(manager_dispatcher, actor_zeta::address_t::empty_address(), command_t::insert, std::string("database_test"),std::string("collection_test"), 1, 5);
